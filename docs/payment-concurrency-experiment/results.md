...
실험을 돌릴 때는 먼저 `.experiment-logs`에 로그를 저장했고, 나중에 남길 로그만 `docs/payment-concurrency-experiment/logs`로 옮겨 두는 방식으로 정리했다. 문서에는 비교에 필요한 결과와 해석만 남긴다.
...
- 마지막 `post-fix-2`는 같은 수정 상태에서 설정을 다시 맞춰 돌린 최종 확인 실험이다.
- 결과는 `APPROVED 100`, `participation 100`, `current_quantity 100`이었고, 공구 상태도 최종적으로 `COMPLETED`까지 전이됐다.
- 따라서 이번 로컬 실험 기준 최종 결과는 `targetQuantity = 50`을 넘긴 뒤에도 `maxQuantity = 100`까지 정상 반영될 수 있음을 확인한 `post-fix-2`로 정리한다.
...
9. 당시 코드 기준으로는 `targetQuantity = 50` 달성 후 `ACHIEVED` 상태로 바뀌면, `maxQuantity = 100`이 남아 있어도 조건부 수량 증가 쿼리가 더 이상 동작하지 않았다.
10. 그래서 처음에는 "50을 넘기면 안 되는 설계인가?"라고 볼 수도 있었지만, 쿼리를 수정한 뒤 `post-fix-1`에서 `73`, `post-fix-2`에서 `100`까지 올라가는 걸 확인하면서 원인이 명확해졌다.
...
- 최종적으로는 `current_quantity = 100`, `APPROVED = 100`, `participation = 100`까지 맞춰 보면서, 어떤 조건이 실제 반영량을 막고 있었는지 실험 결과로 설명할 수 있게 됐다.
