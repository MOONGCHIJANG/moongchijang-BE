...
- **§4.1 사용자 영향 지표 중 하나라도 임계치를 초과하면 즉시 롤백을 결정한다.** 다른 지표 추가 발생을 기다리지 않는다.
- **§4.2 품질 신호 / §4.3 외부 의존** 항목은 단독으로는 롤백을 강제하지 않으며, 사용자 영향과 함께 발생할 때 가중치로 본다. 단, §4.2 항목이 자체 회복 없이 누적되면 롤백을 적극 검토한다.

Gemini 등 자동 폴백이 적용된 외부 의존 장애는 본 섹션의 롤백 조건이 아니라 §5 상시 경보 항목으로 분리해 다룬다.

### 4.1 사용자 영향 (임계치 1개라도 초과 시 즉시 롤백)

- [ ] 검색 API 5xx 비율이 직전 1시간 평균 대비 2배 이상 증가.
- [ ] 검색 평균 응답 시간이 직전 1시간 평균 대비 2배 이상 증가.
- [ ] 결과 카드의 명백한 false positive 사용자 신고 발생.

### 4.2 품질 신호 (단독으로는 롤백 트리거 아님, 누적 시 검토)
...
- [ ] Qdrant 자체가 장기 장애 상태이며 자체 복구가 60분 내 불가하고, 동시에 §4.1 사용자 영향 지표가 악화되는 경우. (Qdrant 단독 장애는 `RetrievalPipeline.searchVectorCandidates` try/catch로 자동 graceful degradation 되므로 §4.1 영향이 없으면 롤백 대상이 아니다.)

## 5. 상시 경보 (롤백 대상은 아님)

자동 폴백이 적용되어 있어 사용자 응답이 정상으로 유지되는 외부 의존 장애는 롤백이 아닌 경보/온콜 대응으로 처리한다.

- [ ] Gemini 호출 실패율이 5%/min 초과하며 회복 기미가 없음. → 런북 §3.2 절차에 따라 원인 분류 / Gemini 측 상태 확인. 검색 자체는 alias fallback + 가드로 계속 동작하므로 롤백이 아니라 상시 점검 사안.
- [ ] LLM 의존 기능 전반이 광범위하게 무력화되어 사용자 의도 추출 품질이 장기간 저하되는 경우 → 별도 결정 회의에서 롤백/기능 토글/대체 LLM 적용을 판단.
...
## 7. Ranking tuning entry criteria
